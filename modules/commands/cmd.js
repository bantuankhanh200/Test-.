module.exports.config = {
    ...
    usages: "[l/u/la/unloadAll/if] [tên module]",
  usePrefix: true,
    cooldowns: 5,
    dependencies: {
        "fs-extra": "",
...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (command.config.envConfig && typeof command.config.envConfig == 'Object') try {
                ...
                throw new Error('Error Loaded: ' + JSON.stringify(error));
            }
            if (command['onLoad']) try {
                ...
                throw new Error('Not Loaded: ' + JSON.stringify(error), 'error');
            }
    ...
    if (errorList.length != 0) api.sendMessage('Error Loaded: ' + errorList.join(' '), threadID, messageID);
...
module.exports.run = function ({ event, args, api }) {
  const { config } = global.config;
    ...
    if (event.senderID != config.OWNER) return api.sendMessage(`Ai cho dùng`, event.threadID, event.messageID)
