...
module.exports.run = async function({ api, event }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`【 ${global.config.PREFIX} 】• ${(!global.config.BOTNAME) ? "Hehe" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		const fs = require("fs");
        await api.sendMessage("Đang thực hiện kết nối...", event.threadID);
        await new Promise(resolve => setTimeout(resolve, 2000));
        await api.sendMessage("〖 This is Zeus Project V2.2... 〗", event.threadID);
        await new Promise(resolve => setTimeout(resolve, 2000));
        await api.sendMessage("〖 Created by Khanh Shado 〗...", event.threadID);
        await new Promise(resolve => setTimeout(resolve, 2000));
        await api.sendMessage(`〖 Zeus Project... Tổng lệnh hiện có là ${global.client.commands.length} 〗`, event.threadID);
        await new Promise(resolve => setTimeout(resolve, 2000));
