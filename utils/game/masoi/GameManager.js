	...
	run(name, gameOptions) {
		...
		for (const item of this.items) {
			...
			if (item.threadID == gameOptions.threadID) {
				...
				return sendMessage(`[⚜️]→ Xin lỗi, box của bạn đang chạy trò chơi ${item.name}!`, gameOptions.threadID);
			}	
			if (item.participants && item.participants.includes(gameOptions.masterID)) {
				return sendMessage(`[⚜️]→ Xin lỗi, bạn đang chơi trò chơi ${item.name}!`, gameOptions.threadID);
			}
		}
