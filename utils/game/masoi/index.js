...
module.exports = class MasoiGame extends Game {
	constructor(options = {}) {
		...
		if (!options.param || isNaN(indexVillage)) {
            var body = `[⚜️]→ Hướng dẫn tạo: ${DEFAULT_THREAD_PREFIX}masoi <mã số làng>\n==============\n\n` +
            '[⚜️]=== Thông tin các làng ===[⚜️]\n\n' +
		...
		if (!gameConfig.setups[indexVillage]) {
            return sendMessage(`[⚜️]→ Không tìm thấy làng với mã số ${symbols[options.param]}!`, this.threadID);
        }
		...
		this.sendMessage(
				...
				`[⚜️]→ Nếu muốn kết thúc game thì nhắn "end!"\n[❗]→ Nếu muốn rời khỏi game thì nhắn "out!"\n` +
	...
	async onMessage(message, reply) {
		...
		if (message.body.toLowerCase() == 'out!') {
			if(!this.participants.includes(message.senderID)) 
				return await this.sendMessage(`[⚠️]→ Bạn chưa tham gia game nên không thể out!\n[⚜️]→ Tình trạng game hiện tại: ${this.participants.length}/${this.setup.roles.length}!`);
			if(message.senderID == this.masterID) 
				...
				for (const player of werewolfParty) {
					if (nameMap.length > 1)
						await player.sendMessage(
							`[⚜️]=== Những người cùng phe với bạn ===[⚜️]\n\n${nameMap
								.filter(name => name != player.name)
								.join(
									', '
								)}\n[⚜️]→ Hãy liên hệ với họ để có 1 teamwork tốt nhất nhé!`
						);
				}
				let balanceScore = 0;
				...
				this.sendMessage(
					this.timing({
						message:
							...
							'\n[⚜️]→ Hãy xem kĩ chi tiết role của mình, trò chơi bắt đầu sau',
